...
public class FileReadingUtil {
	public static void insertSuburbsToDb(Context context, String filePath,
	...
	private static boolean insertSuburdToDb(String suburb,
			DatabaseManager dbManager) {
		// System.out.println("SUBURB IS: "+suburb);
		String[] suburbProperties = suburb.split(",");
		if (suburbProperties.length == 3) {
			DatabaseAccessHelper.updateSuburbData(dbManager,
					suburbProperties[0], suburbProperties[1],
					suburbProperties[2]);
			return true;
		} else {
			return false;
	...
	private static InputStream readSuburbDataFromFile(Context context,
			String filePath) {
		InputStream is = null;
