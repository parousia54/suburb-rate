...
public class RatingAPICaller {
	...
	public static void updateSafety(SuburbStats stats, String ssc) {
		...
		byte[] bytes;
		try {
			bytes = nengine.getRaw(SAFETY_URL + ssc + ".json", null, null,
					30000);
			if (bytes != null) {
	...
	public static void updateSocioEcoId(SuburbStats stats, String ssc) {
		...
		byte[] bytes;
		try {
			bytes = nengine.getRaw(SOCIO_ECO_URL + ssc + ".json", null, null,
					30000);
			if (bytes != null) {
				String result = new String(bytes);
		...
		try {
			...
			if (bytes != null) {
				...
				if (json == null || json.getInt("postcode") != postCode)
					return;
				/*
				 * { ssc: "31565", name: "WEST END", post_code: "4101", state:
				 * "QLD", lga_id: "31000", centre: { lat: "-27.48214186", lon:
				 * "153.0059111" } extents: { ne: { lat: "-27.4742019", lon:
				 * "152.9962674" }, sw: { lat: "-27.4900819", lon: "153.0155549"
				 * } }, surr_subs: { 1: "31265", 2: "31353", ..., 10: "31519" },
				 * boundary:
				 * "http://www.suburbantrends.com.au/api/boundaries/ssc/31565.json"
				 * }
				 */
	...
	public void testJSon() {
		...
		updateSuburb(sub);
		System.out.println(sub.getName() + ", ssc:"
				+ sub.getStatistics().getSsc());
		SuburbStats subst = sub.getStatistics();
		System.out.println("Economic Index:" + subst.getEconomic().getScore()
				+ ", year:" + subst.getDataYear());
		System.out.println("Transport Safety:" + subst.getSafetyTransport()
				+ ", year:" + subst.getSafetyYear());
	}
